...
export default function EditProduct() {
    ...
    const handleSubmit = async (e) => {
      e.preventDefault();
      try {
        const res = await fetch(`/api/product/edit/${formdata._id}`, {
          method: "POST",
    ...
    const handleDelete = async () => {
        // e.preventDefault();
        try {
          const res = await fetch(`/api/product/delete/${formdata._id}`, {
            method: "DELETE",
          });
          const data = await res.json();
          if (data.success === false) {
      ...
      <div className="p-10">
        <form className="flex gap-5" onSubmit={handleSubmit}>
          ...
          <div className="w-1/2  flex flex-col p-4 border-2 border-slate-200 rounded-lg gap-3">
            ...
            <button
              type="Submit"
              className="text-white bg-gradient-to-r from-purple-500 via-purple-600 to-purple-700 hover:bg-gradient-to-br  font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2"
            >
              Save Changes
            </button>
            <button
              ...
              className="text-white bg-gradient-to-r from-red-500 via-red-600 to-red-700 hover:bg-gradient-to-br  font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2"
            >
              Delete
            </button>
          </div>
